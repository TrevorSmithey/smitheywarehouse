...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_canceled_warehouse
ON orders(canceled, warehouse);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_fulfillment_status_canceled
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_line_items_sku
ON line_items(sku);

-- NetSuite wholesale tables indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ns_transactions_tran_date
ON ns_wholesale_transactions(tran_date);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ns_line_items_transaction_id
ON ns_wholesale_line_items(ns_transaction_id);

-- Shipments table indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_shipments_status_delivered_at
ON shipments(status, delivered_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_shipments_status_days_without_scan
