-- Migration: Fix compute_customer_metrics() missing WHERE clause
-- Date: 2026-01-18
--
-- Problem: The segment UPDATE statement had no WHERE clause, causing Supabase
-- to reject it with "UPDATE requires a WHERE clause" error.
--
-- Fix: Add WHERE TRUE to the segment UPDATE statement.
...
BEGIN
  ...
  -- Simplified 3-tier segment classification
  -- FIX: Added WHERE TRUE to satisfy Supabase's WHERE clause requirement
  UPDATE ns_wholesale_customers c
  SET segment = (CASE
...
COMMENT ON FUNCTION compute_customer_metrics IS 'Computes derived fields for wholesale customers. Fixed 2026-01-18: Added WHERE TRUE to segment UPDATE to satisfy Supabase requirements.';
