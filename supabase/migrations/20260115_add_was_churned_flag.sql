-- Migration: Add was_churned flag for reactivation tracking
-- Date: 2026-01-15
--
-- Purpose: Track customers who have EVER been churned (365+ days since last order).
-- This flag never resets to false, enabling us to identify "reactivated" customers:
-- those who were churned but came back.
--
-- Definition:
--   - was_churned = true: Customer has been churned at some point in their history
--   - Reactivated: was_churned = true AND days_since_last_order < 365
--
-- This supports the rolling 12-month churn rate calculation and reactivation tracking.

-- Step 1: Add the column
ALTER TABLE ns_wholesale_customers
ADD COLUMN IF NOT EXISTS was_churned BOOLEAN DEFAULT FALSE;

-- Step 2: Backfill - mark all currently churned customers
-- A customer is churned if they haven't ordered in 365+ days
UPDATE ns_wholesale_customers
SET was_churned = true
WHERE last_sale_date IS NOT NULL
  AND last_sale_date < NOW() - INTERVAL '365 days';

-- Step 3: Update compute_customer_metrics() to set was_churned
-- Key behavior: Once true, never reset to false (captures historical churn state)
...
BEGIN
  ...
  -- NEW 2026-01-15: Set was_churned flag when customer crosses 365d threshold
  -- Important: This flag ONLY sets to true, never resets to false
  -- This allows tracking of "reactivated" customers who were churned but came back
  UPDATE ns_wholesale_customers c
  SET was_churned = true
  WHERE c.days_since_last_order >= 365
    AND c.was_churned = false;

  -- Simplified 3-tier segment classification (updated 2026-01-15)
...
COMMENT ON FUNCTION compute_customer_metrics IS 'Computes derived fields for wholesale customers. Updated 2026-01-15: Added was_churned flag for reactivation tracking. Simplified to 3-tier segments (major >= $20k, mid >= $5k, small < $5k).';

-- Add index for efficient reactivation queries
CREATE INDEX IF NOT EXISTS idx_ns_wholesale_customers_was_churned
ON ns_wholesale_customers(was_churned)
WHERE was_churned = true;
