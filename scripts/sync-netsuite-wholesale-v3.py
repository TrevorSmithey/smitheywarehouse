#!/usr/bin/env python3
"""
NetSuite Wholesale Data Sync V3
- Uses JOIN approach instead of huge IN clause
- Syncs all transactions for business customers (isperson='F', entityid!='493')
- Streams to Supabase as it goes (no memory accumulation)
- Tracks unique transaction IDs to avoid duplicate counting
"""
...
def sync_transactions():
    """Sync wholesale transactions using JOIN approach"""
    print("\n" + "="*70)
    print("SYNCING WHOLESALE TRANSACTIONS")
    print("="*70)
    ...
    print(f"\nFetching transactions for business customers (excluding entity 493)...")
    print("Order: By transaction ID (ASC)\n")

    while True:
        batch_num += 1

        # Use JOIN approach - much more reliable than huge IN clause
        query = {
            "q": f"""
            SELECT DISTINCT
        ...
        if records:
            ...
            if sb_response.status_code not in [200, 201, 204]:
                ...
                print(f"  Batch {batch_num}: {new_count} new transactions | Unique total: {total_transactions:,} | Dates: {date_range}")
        else:
            print(f"  Batch {batch_num}: All {len(items)} transactions were duplicates")
    ...
    print(f"\nTransaction sync complete: {total_transactions:,} unique transactions")
    return total_transactions

def sync_line_items():
    """Sync wholesale line items using JOIN approach"""
    print("\n" + "-"*70)
    print("SYNCING LINE ITEMS")
    print("-"*70)
...
def main():
    print("="*70)
    print("NETSUITE WHOLESALE DATA SYNC V3")
    print("="*70)
